...
runID   = 'bnchm_dt'; % run identifier tag
...
for Nt = NN
    ...
    fh101 = figure(101);
    p1 = loglog(dt,ET,'go','MarkerSize',8,'LineWidth',2); axis xy tight; hold on; box on;
    p2 = loglog(dt,EC,'bv','MarkerSize',8,'LineWidth',2); axis xy tight; hold on; box on;
    ...
    if Nt == NN(1)
        p3 = loglog(dt0.*NN(1)./NN,mean([ET,EC]).*(NN(1)./NN).^2,'k-','LineWidth',2);  % plot linear trend for comparison
    end
    if Nt == NN(end)
        legend([p1,p2,p3],{'error T','error C','quadratic'},'Interpreter','latex','box','on','location','southeast')
    end
