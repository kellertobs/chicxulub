...
runID   = 'bnchm_h'; % run identifier tag
...
for N = NN
    ...
    p2 = loglog(h,ET,'go','MarkerSize',8,'LineWidth',2); axis xy tight; hold on; box on;
    p3 = loglog(h,EC,'bv','MarkerSize',8,'LineWidth',2); axis xy tight; hold on; box on;
    ...
    if N == NN(1)
        ...
        p5 = loglog(D./NN,mean([ET,EC]).*(NN(1)./NN).^2,'k-','LineWidth',2);  % plot linear trend for comparison
    end
    if N == NN(end)
        legend([p1,p2,p3,p4],{'error p','error T','error C','quadratic'},'Interpreter','latex','box','on','location','southeast')
    end
